...
#define TRACE cout << "test: " << SDL_GetTicks() << endl;
#define ENSURE(x, success) if (x != success) {\
		printf("Error: %s\n", SDL_GetError());\
		exit(1); \
	}

#define ENSURE_NOT(x, fail) if (x == fail) {\
		printf("Error: %s\n", SDL_GetError());\
		exit(1); \
	}
...
void drawLineToSurface(SDL_Surface *surface, int x1, int y1, int x2, int y2,
		Uint32 color);
