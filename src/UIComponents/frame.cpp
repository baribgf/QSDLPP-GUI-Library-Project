...
void Frame::drawVScrollBar()
{
	...
	drawLineToSurface(this->baseSurface,
					  this->viewport.w + this->viewport.x - this->scrollBarWidth, 0,
					  this->viewport.w + this->viewport.x - this->scrollBarWidth,
					  this->viewport.h + this->viewport.y,
					  SDL_MapRGBA(this->baseSurface->format, BLACK));

	float ratio = (float)this->viewport.y / height;
	int thumbHeight = ((float)(this->viewport.h - (this->overflowX ? this->scrollBarWidth : 0)) / height) * viewport.h;
	SDL_Surface *c = SDL_CreateRGBSurfaceWithFormat(0,
													this->scrollBarWidth, thumbHeight,
	...
	SDL_Rect r = {this->viewport.w + this->viewport.x - c->w, (int)ceil((height / ((float)(this->viewport.h - (this->overflowX ? this->scrollBarWidth : 0))) + 1) * ratio * (this->viewport.h - (this->overflowX ? this->scrollBarWidth : 0))), c->w, c->h};
...
void Frame::drawHScrollBar()
{
	...
	drawLineToSurface(this->baseSurface, 0,
					  this->viewport.h + this->viewport.y - this->scrollBarWidth, width,
					  this->viewport.h + this->viewport.y - this->scrollBarWidth,
					  SDL_MapRGBA(this->baseSurface->format, BLACK));

	// draw scroll thumb
	float ratio = (float)this->viewport.x / width;
	int thumbWidth = ((float)(this->viewport.w - (this->overflowY ? this->scrollBarWidth : 0)) / width) * viewport.w;
	...
	SDL_Rect r =
		{(int)ceil(
			 (width / ((float)(this->viewport.w - (this->overflowY ? this->scrollBarWidth : 0))) + 1) * ratio * (this->viewport.w - (this->overflowY ? this->scrollBarWidth : 0))),
...
void Frame::invokeEvents(Event e)
{
	if (!this->overrideCompInsideBounds)
	{
		if (this->isOverflowX())
		{
			this->overrideCompInsideBounds = this->insideHScrollBarArea(e.button.x, e.button.y) || this->insideHScrollBarArea(e.wheel.mouseX, e.wheel.mouseY);
		}
		else if (this->isOverflowY())
		{
			this->overrideCompInsideBounds = this->insideVScrollBarArea(e.button.x, e.button.y) || this->insideVScrollBarArea(e.wheel.mouseX, e.wheel.mouseY);
		}
		else
		{
			this->overrideCompInsideBounds = false;
...
void Frame::setScrollBarWidth(int width)
{
...
bool Frame::insideVScrollBarArea(int x, int y)
{
	if (this->parent)
	{
		return this->getAbsPosition().x + this->viewport.w - this->scrollBarWidth - this->parviewport.x <= x && x <= this->getAbsPosition().x + this->viewport.w - this->parviewport.x && this->getAbsPosition().y - this->parviewport.y <= y && y <= this->getAbsPosition().y + this->viewport.h - this->parviewport.y;
	}
	else
	{
		return this->getAbsPosition().x + this->viewport.w - this->scrollBarWidth <= x && x <= this->getAbsPosition().x + this->viewport.w && this->getAbsPosition().y <= y && y <= this->getAbsPosition().y + this->viewport.h;
	}
}
...
bool Frame::insideHScrollBarArea(int x, int y)
{
	if (this->parent)
	{
		return this->getAbsPosition().x - parviewport.x <= x && x <= this->getAbsPosition().x + this->viewport.w - parviewport.x && this->getAbsPosition().y + this->viewport.h - this->scrollBarWidth - parviewport.y <= y && y <= this->getAbsPosition().y + this->viewport.h - parviewport.y;
	}
	else
	{
		return this->getAbsPosition().x <= x && x <= this->getAbsPosition().x + this->viewport.w && this->getAbsPosition().y + this->viewport.h - this->scrollBarWidth <= y && y <= this->getAbsPosition().y + this->viewport.h;
	}
}
